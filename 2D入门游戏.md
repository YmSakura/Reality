...
残影效果应该有很多种实现方法，我们这次采用对象池的方式来实现。

基本思路就是创建一个player的对象池，当按下按键时，主角往前冲，每隔一定的秒数从对象池中取出对象，作为残影。

那么首先我们需要创建一个shadow的prefab，与创建主角的方式相同。
...
主角每次小幅移动之前都在当前位置生成一个shadow，这个shadow的颜色会逐渐变浅直至消失。

每次冲刺如果都重复地生成销毁shadow的话，肯定非常损耗性能，所以我们可以创建一个对象池，每当需要使用的时候，就从对象池中取出对象，不用的时候就放回去，而无需频繁地生成销毁。

用一个空物体作为对象池即可，使池中对象都为它的子物体。

基本思路就是用一个队列存放所有的shadow，初始化时就创建一定数量的shadow出来，使用的时候如果不够了再扩充，需要使用的时候出队一个shadow就可以了。
...
写好了好脚本别忘了回到Unity设置这些参数，然后就是Player的部分：
...
Player这里的逻辑是：当按下冲刺键，判断CD是否结束，如果结束则可以进行冲刺，重置冲刺剩余时间和lastDash的时间点，然后进行Dash，每次在当前位置生成一个shadow，然后移动一定的距离，并用剩余时间减去当前帧消耗的时间，当没有剩余时间时结束冲刺。



这里为了实时显示CD时间，创建了一个简单的UI，具体思路就是用两张图片，一张作为“背景”，另一张作为冷却的图片，将其颜色设置为黑色半透明。

<img src="https://image-ym.oss-cn-guangzhou.aliyuncs.com/img/image-20211119134551532.png" alt="image-20211119134551532" style="zoom:67%;" />

选择image type为filled，可以自行选择fill method和fill origin，然后最重要的是fill amount，当它为1时就是技能图标就是灰色状态，当为0时就是可使用状态，游戏开始时我们当然将其设置为0，那么要怎么让玩家在使用技能后图标变暗（player的代码已经写了），并根据时间开始恢复呢？查询fill amount的api，我们可以看到官方给出了解决方案。
