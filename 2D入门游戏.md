...
然后添加暂停菜单，菜单需要背景和按钮，所以新建一个Panel，并在其下建立几个Button和一个silder（滑动条）
...
然后就是按钮的逻辑部分，我们可以直接在Menu这个类中去添加函数
...
然后把Menu挂到Canvas上，给按钮选择on click event
...
通过修改timeScale可以实现控制游戏速度的效果，如果要实现慢动作就可以使用它。
